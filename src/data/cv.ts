export const experiences = [
	{
		company: 'Research Institution',
		time: '2020 - Present',
		title: 'Senior Research Scientist',
		location: 'City, Country',
		description: 'Leading research projects in artificial intelligence and machine learning, collaborating with interdisciplinary teams.',
	},
	{
		company: 'University Name',
		time: '2018 - 2020',
		title: 'Postdoctoral Researcher',
		location: 'City, Country',
		description: 'Conducted independent research and supervised graduate students in computational methods.',
	},
	// {
	// 	company: 'Radium Institute (Institut du Radium)',
	// 	time: '1914 - 1934',
	// 	title: 'Director',
	// 	location: 'Paris, France',
	// 	description: 'Led groundbreaking studies on radioactivity and mentored future Nobel Prize laureates.',
	// },
];

export const education = [
	{
		school: 'University Name',
		time: '2014 - 2018',
		degree: 'Ph.D. in Computer Science',
		location: 'City, Country',
		description: 'Dissertation: "Advanced Topics in Machine Learning and Data Analysis"',
	},
	{
		school: 'University Name',
		time: '2012 - 2014',
		degree: 'Master\'s in Computer Science',
		location: 'City, Country',
		description: 'Specialized in artificial intelligence and computational methods.',
	},
	// {
	// 	school: 'University of Paris',
	// 	time: '1891 - 1895',
	// 	degree: 'Master's in Physics and Mathematics',
	// 	location: 'Paris, France',
	// 	description: 'Graduated at the top of her class in physics and second in mathematics.',
	// },
];

export const skills = [
	{
		title: 'Programming Tools & Languages',
		description: 'Python • Java • C/C++ • JavaScript • TypeScript • SQL • Bash • Assembly • HTML/CSS • CUDA • MATLAB',
	},
	{
		title: 'Machine Learning Tools',
		description: 'Deep Learning (PyTorch) • LLM Fine-tuning (Hugging Face) • Model Quantization (GPTQ/AWQ) • Reinforcement Learning (PyTorch) • Classical ML (Scikit-Learn) • AI Safety Evaluation (HELM) • GPU Optimization (CUDA) • API Integration (OpenAI, Anthropic)',
	},
	{
		title: 'Development & Research Tools',
		description: 'Data Analysis (Pandas, NumPy) • Visualization (Matplotlib, D3.js) • Web Development (React, Flask) • Database Management (PostgreSQL, MySQL) • Containerization (Docker) • Version Control (Git/GitHub) • HPC Computing (Slurm) • Scientific Computing (SciPy, Jupyter)',
	},
];

export const publications = [
	{
		title: 'Hallucination and Calibration in Language Models',
		authors: 'Harutyun Harry Ilanyan',
		journal: 'UC Berkeley',
		time: '2025',
		link: 'https://wp.math.berkeley.edu/drp/wp-content/uploads/sites/18/2025/05/2025_Spring_Ilanyan.pdf',
		abstract: 'Examines why perfectly calibrated language models must inherently produce some factual "hallucinations" due to rare or one-off training facts, showing how strategic data duplication and controlled miscalibration can effectively reduce hallucination rates in practice.',
		tags: ['AI Safety', 'Interpretability']
	},
];
